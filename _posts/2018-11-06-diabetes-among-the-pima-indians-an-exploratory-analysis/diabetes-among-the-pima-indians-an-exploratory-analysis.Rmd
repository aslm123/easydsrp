...
Diabetes <- read_csv("C:/Users/aselr/Documents/RMIT/SEMESTER 2 2018/Analysis of Categorical Data/Final Project/Phase 1/diabetes.csv") %>% 
  as.data.frame()
```
The reason to add `as.data.frame()` is that sometimes the data will be read into a list format but we want it in a data frame format for this project.
...
need to be converted into numeric variables from its current integer class. These variables have decimal values and its absecnce could potentially create misleading results and distort specified ranges of risk represented by those medical tests.
...
## Numeric Variables {.tabset .tabset-fade}
...
## Categorical Variables (% of Outcome) {.tabset .tabset-fade}
